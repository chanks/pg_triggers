  ...
  describe "with a parent_condition argument" do
    before do
      DB.run \
        PgTriggers.conditional_foreign_key(
          ...
          child_table: :children,
          relationship: {id1: :parent_id1, id2: :parent_id2},
          parent_condition: "ROW.other_column >= 5",
        )

      DB[:parents].insert(id1: 1, id2: 3, other_column: 10)
      DB[:parents].insert(id1: 2, id2: 4, other_column: 10)
      DB[:children].insert(parent_id1: 1, parent_id2: 3)
    end

    describe "when modifying a parent" do
      describe "with an update" do
        it "should not throw an error when updating a parent key that has no referring children" do
          DB[:parents].where(id1: 2).update(id1: 3).should == 1
        end

        it "should not throw an error when updating a parent that has no referring children out of the condition" do
          DB[:parents].where(id1: 2).update(other_column: 1).should == 1
        end
        ...
        it "should not throw an error when updating non-key columns of a parent that a child is referring to" do
          DB[:parents].where(id1: 1, id2: 3).update(other_column: 12).should == 1
        end

        it "should throw an error when updating parent out of a condition that a child is pointing to" do
          key = [:id1, :id2].sample
          # TODO: Figure out what to raise to make these ForeignKeyConstraint violations.
          proc{DB[:parents].where(id1: 1).update(other_column: 1)}.should raise_error(Sequel::DatabaseError, /update in parents violates foreign key constraint/)
        end
    ...
    describe "when modifying a child" do
      describe "with an insert" do
        ...
        it "should throw an error when the specified parent exists without the condition" do
          pending
          DB[:parents].where(id1: 2).update(other_column: 2)
          proc{DB[:children].insert(id: 2, parent_id1: 2, parent_id2: 4)}.should raise_error(Sequel::DatabaseError, /insert in children violates foreign key constraint/)
        end
      ...
      describe "with an update" do
        ...
        it "should throw an error when updating a child's foreign key when the new parent doesn't meet the condition" do
          pending
          DB[:parents].where(id1: 2).update(other_column: 2)
          proc{DB[:children].where(id: 1).update(parent_id1: 2, parent_id2: 4)}.should raise_error(Sequel::DatabaseError, /update in children violates foreign key constraint/)
        end
